...
//결제금액렌더
const $allChoiceProductPrice = document.querySelector(
  ".all-choice-product-price"
);
renderPaymentAmount();
...
function increaseCount(e) {
  ...
  item.querySelector(".bp_price>p").innerHTML =
    `${(target.count * target.price).toLocaleString("ko-KR")}원`;
  localStorage.setItem("basketItemList", JSON.stringify(basketItemList));
  renderPaymentAmount();
}
...
function openOptionModal(e) {
  ...
  $optionModal.querySelector(".om_price>p").innerHTML =
    `${(target.count * target.price).toLocaleString("ko-KR")}원`;
  ...
  for (let key in targetAllOption) {
    ...
    $optionModal.querySelector(".om_choice-wrap").innerHTML += createAllOption(
      optionName,
      optionArr
    );
  }
...
function increaseCountModal(e) {
  ...
  item.querySelector(".om_price>p").innerHTML =
    `${(currentCount * target.price).toLocaleString("ko-KR")}원`;
}
  ...
  selects.forEach((el) => {
    el.addEventListener("change", () => {
      ...
      if (selects.length == Object.keys(option).length) {
        ...
        $optionModal.querySelector(".om_option-list>p").innerHTML =
          allchoiceOption;
      }
    });
  });
...
function updateOption(e) {
  ...
  selects.forEach((el) => {
    currentChoiceOption[el[0].innerText] =
      target.allOption[el[0].innerText][el.value];
  });
